...
class StaffGradedAssignmentXBlock(
    ...
    grader_email = String(
        display_name="Grader Email",
        default="grading@gbc-college.com",
        scope=Scope.settings,
        help="Grader Email",
    )

    instructor_email = String(
        display_name="Instructor Email",
        default="grading@gbc-college.com",
        scope=Scope.settings,
    ...
    def save_sga(self, data, suffix=""):
        ...
        self.grader_email = data.get("grader_email", self.grader_email)
        self.instructor_email = data.get("instructor_email", self.instructor_email)
    ...
    def upload_assignment(self, request, suffix=""):
        ...
        stu_id = self.xmodule_runtime.anonymous_student_id
        user = user_by_anonymous_id(stu_id)
        ...
        student_profile_name = module.student.profile.name
        # course_name = self.get_course_name_or_id(self.course_id)
        course_names = str(self.course_id)
        data = course_names.split(":")
        course_name = data[1]
        ...
        message = (
            "Hare Krishna, <br>An assignment has been submitted for the course <b>"
            + course_name
            + "</b> by <b>"
            + student_profile_name
            + ".</b> Uploaded Asignment is <b>"
            + upload_filename
            + ".</b> You can access this assignment by logging in."
        )
        message = message + "<br><br>Thank You,<br>GBC College Staff"
        subject = "An assignment uploaded by a student"
        fromemail = "Courses | GBC College <courses@gbc-college.com>"
        all_emails = self.grader_email.split(",")
        for email1 in all_emails:
            if email1 != "":
                try:
                    send_mail(
                        subject,
                    ...
                    log.info(
                        "Assignments Uploaded email sent sucessfully to {}".format(
                            email1
                        )
                    )
                except Exception as e:
                    log.info(
                        "Failed to send Assignments Uploaded email to {}. Error: {}".format(
                            email1, str(e)
                        )
                    )
        log.info(
    ...
    def enter_grade(self, request, suffix=""):
        ...
        current_user_profile_name = module.student.profile.name
        module = StudentModule.objects.get(pk=request.params["module_id"])
        student_profile_fullname = module.student.profile.name
        student_profile_username = module.student.username
        student_email = module.student.email
        ...
        staff_grading_data1 = self.staff_grading_data()
        staff_grading_data2 = staff_grading_data1["assignments"]
        for sgd in staff_grading_data2:
            if str(sgd["username"]) == student_profile_username:
                uploaded_assignment_name = sgd["filename"]

        if self.is_instructor():
            message = (
                "Hare Krishna, <br>Your assignment for the course <b>"
                + course_name
                + "</b> and uploaded assignment filename <b>"
                + uploaded_assignment_name
                + "</b> has been graded."
            )
            message = message + "<br><br>Thank You,<br>GBC College Staff"
            ...
            if student_email != "":
                try:
                    ...
                    log.info(
                        "Grade Uploaded email sent sucessfully to {}".format(
                            student_email
                        )
                    )
                except Exception as e:
                    log.info(
                        "Failed to send Grade Uploaded email to {}. Error: {}".format(
                            student_email, str(e)
                        )
                    )
        if self.is_course_staff():
            message = (
                "Hare Krishna, <br>A student named <b>"
                + student_profile_fullname
                + "</b> has been graded by <b>"
                + current_user_profile_name
                + "</b> for the course <b>"
                + course_name
                + "</b> and uploaded assignment filename is <b> "
                + uploaded_assignment_name
                + "</b>"
            )
            message = message + "<br><br>Thank You,<br>GBC College Staff"
            ...
            fromemail = "Courses | GBC College <courses@gbc-college.com>"
            all_emails1 = self.instructor_email.split(",")
            for email2 in all_emails1:
                if email2 != "":
                    try:
                        send_mail(
                            subject,
                        ...
                        log.info(
                            "Grade Uploaded email sent sucessfully to {}".format(email2)
                        )
                    except Exception as e:
                        log.info(
                            "Failed to send Grade Uploaded email to {}. Error: {}".format(
